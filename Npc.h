...
public:
	...
	int  m_iHP;						// Hit Point 
	unsigned long  m_iExp;                    // 죽였을 경우 얻는 경험치. ExpDice에 의해 결정된다.
	...
	unsigned long m_iExpDiceMin;
	unsigned long m_iExpDiceMax;
	int  m_iGoldDiceMin;
	int  m_iGoldDiceMax;
	...
	BOOL  m_bIsPermAttackMode;		// 이 플래그가 활성화 되어 있으면 공격도중 목표를 바꾸지 않는다.
   	unsigned long   m_iNoDieRemainExp;		// 죽지 않고 명중시에 얻을 수 있는 경험치 
