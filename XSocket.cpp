...
int XSocket::_iRegisterUnsentData(char * cData, int iSize)
{
	...
	SafeCopy(m_pUnsentDataList[m_sTail], cData, iSize);
 ...
 //char   m_msgBuff[50000], dataBuff[50000];
	...
	*wp = (WORD)dwSize + 3;
	...
	SafeCopy((char *)(m_pSndBuffer + 3), cData, dwSize);
        ...
        wsprintf(m_msgBuff,"XSocket message of size %lu was sent = ", dwSize);
        SafeCopy(dataBuff, cData, dwSize);
...
int XSocket::iGetPeerAddress(char * pAddrString)
{
 ...
 int iRet, iLen;
	...
	SafeCopy(pAddrString, (char *)inet_ntoa(sockaddr.sin_addr), 0);
